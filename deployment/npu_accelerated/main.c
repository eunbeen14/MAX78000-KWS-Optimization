...
// 수정 후: 32-bit 최적화를 위한 4-byte 정렬 강제
...
static int16_t ml_data_16[32]; //추가
    ...
    while (1) {
        ...
        if (procState == SILENCE) {
            ...
            if (avg >= thresholdHigh) {
                ...
                // =========================================================
                // [Optimization] 4-byte Alignment via Bit Masking
                // Forces the start index to align with a 4-byte boundary.
                // This is critical for the 32-bit casted AddTranspose function 
                // to avoid unaligned memory access (HardFault) on ARM Cortex-M4.
                // =========================================================                
...
#endif //#ifdef ENABLE_SILENCE_DETECTION
        {
            ...
            if (ai85Counter >= SAMPLE_SIZE) {
                ...
                // =========================================================
                // [Profiling] Measure NPU Data Formatting Overhead
                // Resetting SysTick to count Cortex-M4 cycles consumed during 
                // the heavily optimized AddTranspose (Unrolling) operations.
                // =========================================================
                SysTick->LOAD = 0xFFFFFF; // 24-bit max
                SysTick->VAL = 0;         
                SysTick->CTRL = 5;        // Enable timer
                ...
                // =========================================================
                // [Profiling Result] Data Formatting Latency
                // =========================================================
                uint32_t end_mcu = SysTick->VAL;
                uint32_t mcu_ticks = (start_mcu - end_mcu) & 0xFFFFFF;
                // Convert cycles to microseconds (Assuming 100MHz Core Clock)
                printf("\n====================================\n");
                printf("[NPU Pre-processing] Data Format Time: %lu us\n", mcu_ticks / (SystemCoreClock / 1000000));
                printf("====================================\n");
...
// =========================================================================
// [Optimization: NPU Memory Bottleneck Reduction]
// 32-bit Unrolling & Bitwise Routing for Transpose Operation
// 
// Description:
// The MAX78000 CNN Accelerator requires 1D audio data to be interleaved 
// and transposed across 4-byte word boundaries (e.g., 0x0000 -> 0x0400).
// The baseline AddTranspose function used modulo (%) and division (/) which 
// caused severe latency overheads on the Cortex-M4 core.
//
// Solution:
// 1. Pointer Casting: Casted 8-bit arrays to 32-bit pointers to process 
//    4 bytes simultaneously (Memory Unrolling).
// 2. Bitwise Math: Replaced costly math operations with bitwise shifts (<<, &).
// 3. Memory Alignment: Ensured 4-byte alignment to prevent HardFault exceptions.
// =========================================================================
uint8_t AddTranspose(uint8_t *pIn, uint8_t *pOut, uint16_t inSize, uint16_t outSize, uint16_t width)
{
    static uint32_t row = 0;
    static uint32_t c = 0;      // 32-bit word horizontal index
    static uint32_t total = 0;
    
    // [Optimization] Pointer casting for 32-bit bulk memory access
    uint32_t *pOut32 = (uint32_t *)pOut;
    uint32_t *pIn32 = (uint32_t *)pIn;
    
    // inSize (CHUNK) is 128 bytes, processing 32 words at a time
    uint32_t wordsToProcess = inSize >> 2; 

    for (uint32_t i = 0; i < wordsToProcess; i++) {
        uint32_t out_word_idx;
        
        // [Optimization] Math operations replaced by bitwise shifts
        if (c < 16) { 
            out_word_idx = (c << 8) + (row << 1);            
        } else {      
            out_word_idx = ((c - 16) << 8) + (row << 1) + 1; 
        }
        
        // [Hardware Alignment] Safely route 4-bytes directly to NPU memory offset.
        // Requires pIn array to be __attribute__((aligned(4)))
        pOut32[out_word_idx] = pIn32[i];
