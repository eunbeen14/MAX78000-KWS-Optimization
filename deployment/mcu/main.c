...
__attribute__((aligned(4))) int8_t micBuff[SAMPLE_SIZE];
...
int8_t mcu_input_buffer[SAMPLE_SIZE];
int8_t mcu_output_buffer[NUM_OUTPUTS];
    ...
    while (1) {
        ...
        if (procState == SILENCE) {
            ...
            if (avg >= thresholdHigh) {
                ...
                // 수정: 비트 마스킹을 통한 4의 배수 내림
                utteranceIndex = micBufIndex & ~3;
...
#endif //#ifdef ENABLE_SILENCE_DETECTION
        {
            ...
            /* If enough samples are collected, start MCU inference */
            if (ai85Counter >= SAMPLE_SIZE) {
                int16_t out_class = -1;
                double probability = 0;
                
                // Reset SysTick for potential cycle counting
                SysTick->LOAD = 0xFFFFFF; 
                SysTick->VAL = 0;         
                SysTick->CTRL = 5;        
                uint32_t start_mcu = SysTick->VAL;

                /* Calculate the end of the utterance */
                ...
                // =====================================================================
                // [Step 1: Buffer Flattening & Zero Padding]
                // Description: Unrolls the circular microphone buffer (Ring Buffer) into 
                // a strictly sequential 1D array required by CMSIS-NN. Bypasses the 
                // NPU-specific AddTranspose hardware formatting overhead.
                // =====================================================================
                int start_idx = utteranceIndex - PREAMBLE_SIZE;
                if (start_idx < 0) {
                    start_idx += SAMPLE_SIZE;
                }

                // Sequential copy for CMSIS-NN input
                for (int i = 0; i < SAMPLE_SIZE - zeroPad; i++) {
                    mcu_input_buffer[i] = micBuff[(start_idx + i) % SAMPLE_SIZE];
                }
                
                // Zero Padding to match the model's fixed input dimension (128x128)
                for (int i = SAMPLE_SIZE - zeroPad; i < SAMPLE_SIZE; i++) {
                    mcu_input_buffer[i] = 0; 
                }

                /* Reset audio processing counters for the next utterance */
                ai85Counter = 0;
                avgSilenceCounter = 0;
                wordCounter++;
                procState = SILENCE;
                ...
                // =====================================================================
                // [Step 2: MCU Inference (CMSIS-NN) & Profiling]
                // Description: Executes the pure software ResNet-lite model on the 
                // ARM Cortex-M4 core and profiles the exact execution time in microseconds.
                // =====================================================================
                MXC_TMR_SW_Start(MXC_TMR0); // Start hardware timer

                run_mcu_resnet_lite(mcu_input_buffer, mcu_output_buffer);

                uint32_t mcu_usec = MXC_TMR_SW_Stop(MXC_TMR0); // Stop and read microseconds

                printf("\n====================================\n");
                printf("[MCU Baseline] Inference Time: %u us\n", mcu_usec);
                printf("====================================\n");

                // =====================================================================
                // [Step 3: Softmax Post-processing & Confidence Thresholding]
                // Description: 
                // The MAX78000 NPU (or CMSIS-NN) outputs raw logits (int8_t). 
                // We use the Cortex-M4 core to compute the Softmax probabilities 
                // to evaluate the confidence score of the Keyword Spotting result.
                // =====================================================================
                
                // 1. Convert int8_t logits to q31_t (32-bit) for Softmax calculation
                q31_t ml_data32[NUM_OUTPUTS];
                for (int i = 0; i < NUM_OUTPUTS; i++) {
                    ml_data32[i] = (q31_t)mcu_output_buffer[i];
                }

                // 2. Execute ARM CMSIS optimized Softmax
                // (Assuming softmax_q17p14_q15 is available via softmax.c)
                q15_t ml_softmax[NUM_OUTPUTS];
                softmax_q17p14_q15(ml_data32, NUM_OUTPUTS, ml_softmax);

                // 3. ArgMax: Find the class with the highest probability
                out_class = 0;
                q15_t max_prob = 0;
                for (int i = 0; i < NUM_OUTPUTS; i++) {
                    if (ml_softmax[i] > max_prob) {
                        max_prob = ml_softmax[i];
                        out_class = i;
                    }
                }

                // 4. Convert Q15 format (0 to 32768) to human-readable percentage (0.0 to 100.0%)
                probability = (double)max_prob * 100.0 / 32768.0;

                printf("Predicted Class: %d, Confidence: %.1f%%\n", out_class, probability);

                // 5. Thresholding (e.g., Confidence must be > 75%)
                if (probability < 75.0) {   
                    ret = 0;         // Unknown / Below Threshold
                } else {
                    ret = 1;         // Keyword Detected confidently
                }
